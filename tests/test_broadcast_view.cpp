...
TEST_F(BroadcastViewTest, CanBroadcastCheck) {
    // Compatible shapes
    EXPECT_TRUE((can_broadcast<3>()));        // [3] can broadcast with anything
    EXPECT_TRUE((can_broadcast<1, 3>()));     // [1, 3] can broadcast
    EXPECT_TRUE((can_broadcast<3, 1>()));     // [3, 1] can broadcast
    EXPECT_TRUE((can_broadcast<3, 3>()));     // Same shape always works

    // Incompatible shapes
    EXPECT_FALSE((can_broadcast<3, 2>()));    // [3] and [2] incompatible
    EXPECT_FALSE((can_broadcast<2, 3, 3, 2>())); // [2, 3] and [3, 2] incompatible
}
